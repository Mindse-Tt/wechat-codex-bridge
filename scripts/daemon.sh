...
# =============================================================================
# wechat-codex-bridge cross-platform daemon manager
...
PLIST
  ...
  echo "Started wechat-codex-bridge daemon (macOS launchd)"
}
...
macos_stop() {
  ...
  rm -f "$plist_path"
  echo "Stopped wechat-codex-bridge daemon (macOS launchd)"
}
...
linux_direct_start() {
  ...
  echo "Starting wechat-codex-bridge daemon (direct mode)..."
...
linux_start() {
  if linux_systemd_available; then
    ...
    echo "Started wechat-codex-bridge daemon (Linux systemd)"
  else
...
linux_stop() {
  if linux_systemd_available && systemctl --user cat "${SERVICE_NAME}" &>/dev/null; then
    ...
    echo "Stopped wechat-codex-bridge daemon (Linux systemd)"
  else
    linux_direct_stop
  fi
}
