 ...
 */
export async function downloadImageFile(item: MessageItem, targetDirectory?: string): Promise<string | null> {
  ...
  try {
    ...
    const tmpDir = targetDirectory ?? path.join(os.tmpdir(), 'wechat-codex-bridge');
    fs.mkdirSync(tmpDir, { recursive: true });
    const filePath = path.join(tmpDir, `image-${Date.now()}-${Math.random().toString(36).slice(2)}.${extension}`);
    fs.writeFileSync(filePath, decrypted);
    logger.info('Image downloaded and decrypted', { size: decrypted.length, filePath });
    return filePath;
  } catch (err) {
 ...
 */
export async function downloadFile(item: MessageItem, targetDirectory?: string): Promise<string | null> {
  ...
  try {
    ...
    const tmpDir = targetDirectory ?? path.join(os.tmpdir(), 'wechat-codex-bridge');
    fs.mkdirSync(tmpDir, { recursive: true });
    const suppliedName = path.basename(fileItem.file_name || `file-${Date.now()}.bin`);
    const filePath = path.join(tmpDir, `file-${Date.now()}-${Math.random().toString(36).slice(2)}-${suppliedName}`);
    fs.writeFileSync(filePath, decrypted);
    logger.info('File downloaded and saved', { path: filePath, size: decrypted.length, name: suppliedName });
    return filePath;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
