...
export class WeChatApi {
  ...
  private async request<T = Record<string, unknown>>(
    ...
    // Message bodies and context tokens can contain private chat content or credentials.
    logger.debug('API request', { url, operation: path });
    ...
    try {
      ...
      logger.debug('API response', summarizeResponse(path, json));
      return json;
    } catch (err) {
...
function summarizeResponse(path: string, response: unknown): Record<string, unknown> {
  ...
  return {
    ...
    messageCount: Array.isArray(value.msgs) ? value.msgs.length : undefined,
  };
}
