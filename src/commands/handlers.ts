...
import { scanAllSkills, formatSkillList, findSkill, type SkillInfo } from '../codex/skill-scanner.js';
...
export function handleCwd(ctx: CommandContext, args: string): CommandResult {
  if (args) {
    return { reply: '为保护本机文件，微信端不能切换工作目录。请在电脑本地修改配置后重启服务。', handled: true };
  }
  return { reply: `当前受限工作目录: ${ctx.session.workingDirectory}`, handled: true };
}
...
export function handleCompact(ctx: CommandContext): CommandResult {
  ...
  if (!currentSessionId) {
    return { reply: 'ℹ️ 当前没有活动的 Codex 会话，无需新开。', handled: true };
  }
  ctx.updateSession({
  ...
  return {
    reply: '✅ 下次消息将开始新的 Codex 会话；聊天历史已保留，可用 /history 查看。',
    handled: true,
  };
}
...
export function handleVersion(): CommandResult {
  try {
    ...
    return { reply: `wechat-codex-bridge v${version}`, handled: true };
  } catch {
    return { reply: 'wechat-codex-bridge (version unknown)', handled: true };
  }
}
...
export function handleSend(ctx: CommandContext, args: string): CommandResult {
  if (!args) {
    return { reply: '用法: /send <工作目录内的相对路径>\n例: /send outputs/report.pdf\n     /send chart.png', handled: true };
  }

  const resolved = resolve(ctx.session.workingDirectory, args);
  if (!existsSync(resolved)) {
    return { reply: `文件不存在: ${resolved}`, handled: true };
  }

  const workspace = realpathSync(resolve(ctx.session.workingDirectory));
  const actualPath = realpathSync(resolved);
  const relativePath = relative(workspace, actualPath);
  if (relativePath.startsWith('..') || isAbsolute(relativePath)) {
    return { reply: '只能发送受限工作目录中的文件。', handled: true };
  }

  const stat = statSync(actualPath);
  if (stat.isDirectory()) {
    return { reply: `这是一个目录，请指定文件: ${actualPath}`, handled: true };
  }
  ...
  return { handled: true, sendFile: actualPath };
}
...
export function handleUnknown(cmd: string, args: string): CommandResult {
  ...
  if (skill) {
    const prompt = args ? `请使用 ${skill.name} skill：${args}` : `请使用 ${skill.name} skill。`;
